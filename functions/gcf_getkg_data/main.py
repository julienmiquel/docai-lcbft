...
def extract_kg_info(query,data_type='json'):
  ...
  'query': query,
  'limit': 1,
  'types': ['Organization', 'GovernmentOrganization', 'Person'],
  'indent': True,
  'key': API_key,
  }
  print(query)
  ...
  if ('itemListElement' in response_fields) and (len(r.json()['itemListElement']) > 0):
    response = r.json()['itemListElement'][0] 
  else:
    print("No items in Knowledge Graph response.")
    print(r.json())
    return {}
