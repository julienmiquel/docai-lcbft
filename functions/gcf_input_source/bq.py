...
def write_to_bq(entities_extracted_dict, dataset_name=var.dataset_name, table_name=var.table_name):
    ...
    from google.cloud.bigquery.retry import DEFAULT_RETRY, _should_retry
    ...
    while(retryCount < 3):
        try:
            ...
            for e in job.errors:
                print('BQ ERROR Forbidden: {}'.format(e)) 
                logging.Logger.error('BQ ERROR: {}'.format(e)) 
                if retryCount >= 3:
                    logging.Logger.fatal('BQ FATAL ERROR ') 
                    raise e
